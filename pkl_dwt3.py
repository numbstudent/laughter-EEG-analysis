...
for yy in range(0,len(sn)):
    with open('Z:/nani/experiment/'+sn[yy]+'/dry laugh_yes.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
        d1 = pickle.load(f)
    data = d1[0]
    channel = 4
    data = np.array(data)
    ...
    for xx in range(0,len(data)):
        ...
        freqs_mt.append(ee)
        psd_mt.append(ff)
    # with open('Z:/nani/experiment/'+sn[x]+'/dry laugh_no.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
    #     d1 = pickle.load(f)
    # data = d1[0]
    # channel = 4
    # data = np.array(data)
