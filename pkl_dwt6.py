...
def run(channel):
    ...
    jenis = 'short no laugh 2'

    with open('Z:/nani/experiment/TP_'+jenis+'.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
        d1 = pickle.load(f)
    data = d1[0]
    # channel = 4
    data = np.array(data)
    ...
    for xx in range(0,len(data)):
        aa,bb,cc,dd,ee,ff = plot_spectrum_methods(data[xx], sf, 5, band, dB)
        freqs.append(aa)
        psd.append(bb)
    ...
    # Start plot
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(12, 5), sharex=True, sharey=sharey)
    # Stem
    sc = 'red'
    ...
    freqs_mt = []
    psd_mt = []
    with open('Z:/nani/experiment/TN_'+jenis+'.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
        d1 = pickle.load(f)
    data = d1[0]
    # channel = 4
    data = np.array(data)
    ...
    ax1.set_title(title[channel]+' Periodogram')
    ax2.set_title(title[channel]+' Welch')
    ax3.set_title(title[channel]+' Multitaper')
    if band is not None:
        ax1.set_xlim(band)
    ...
    # plt.show()
    # exit()
    plt.savefig('wkowko '+jenis+' ch'+str(channel)+'.jpg')
    plt.close()
